...
if __name__ == "__main__":
    original_image = load_image("data/grey12_1/camera.tif")
    original_image = add_noise(original_image)

    # encoder = RANSACEncoder(patch_size=4, degree=3, threshold=30)
    encoder = MeanEncoder()
    # sampler = ImportanceSampler(iterations=3, patch_size=2)
    # sampler = MCMCSampler()

    # encoder = SVDEncoder(k=20, patch_size=8, threshold=200)
    # sampler = ImportanceSampler(iterations=2, k=2)
    sampler = MeanSampler(patch_size=2)
